...
详细财务指标：
【盈利能力】
- 净资产收益率(ROE)：{ratios.get('净资产收益率ROE', ratios.get('ROE', None))}
- 总资产收益率(ROA)：{ratios.get('总资产收益率ROA', ratios.get('ROA', None))}
- 销售毛利率：{ratios.get('销售毛利率', ratios.get('毛利率', None))}
- 销售净利率：{ratios.get('销售净利率', ratios.get('净利率', None))}

【偿债能力】
- 资产负债率：{ratios.get('资产负债率', None)}
- 流动比率：{ratios.get('流动比率', None)}
- 速动比率：{ratios.get('速动比率', None)}

【运营能力】
- 存货周转率：{ratios.get('存货周转率', None)}
- 应收账款周转率：{ratios.get('应收账款周转率', None)}
- 总资产周转率：{ratios.get('总资产周转率', None)}

【成长能力】
- 营业收入同比增长：{ratios.get('营业收入同比增长', ratios.get('收入增长', None))}
- 净利润同比增长：{ratios.get('净利润同比增长', ratios.get('盈利增长', None))}

【每股指标】
- 每股收益(EPS)：{ratios.get('EPS', None)}
- 每股账面价值：{ratios.get('每股账面价值', None)}
- 股息率：{ratios.get('股息率', stock_info.get('dividend_yield', None))}
- 派息率：{ratios.get('派息率', None)}
"""
...
重点审查利润质量：如果经营性现金流净额远低于净利润，必须质询是否存在业绩造假。结合季报数据，识别业绩增长是否已‘见顶’，防止在景气度最高点买入循环性行业。
【基本信息】
- 股票代码：{stock_info.get('symbol', None)}
- 股票名称：{stock_info.get('name', None)}
- 当前价格：{stock_info.get('current_price', None)}
- 市值：{stock_info.get('market_cap', None)}
- 行业：{stock_info.get('sector', None)}
- 细分行业：{stock_info.get('industry', None)}

【估值指标】
- 市盈率(PE)：{stock_info.get('pe_ratio', None)}
- 市净率(PB)：{stock_info.get('pb_ratio', None)}
- 市销率(PS)：{stock_info.get('ps_ratio', None)}
...
关注大单流出但股价小涨（诱多），或者特大单突然砸盘。
特别预警：若主力资金在股价拉升过程中持续净流出，必须定义为‘边打边撤’的派发模式。分析主力与散户博弈：如果主力撤离而散户疯狂涌入，不论涨幅多大，风险评定直接设为‘极高’。
【基本信息】
股票代码：{stock_info.get('symbol', None)}
股票名称：{stock_info.get('name', None)}
当前价格：{stock_info.get('current_price', None)}
...
【技术指标】
- 量比：{indicators.get('volume_ratio', None)}
- 当前成交量与5日均量比：{indicators.get('volume_ratio', None)}
{fund_flow_section}
...
- 注意区分短期波动与趋势性变化
- 在分析趋势时，必须对比分时白线与黄线的位置关系。若白线在黄线下方，所有的 KDJ 低位金叉必须定义为‘弱势反抽’而非‘买入信号’。增加‘偏离度’检测：若股价远离黄线超过 3%，提示‘乖离率过高，严禁追涨，等待回踩’。
...
请模拟一场激烈的投委会辩论。引入[首席质疑官]席位，他必须针对其他分析师的乐观观点提出至少 2 条反向思维质询（例如：如果现在的利好是‘利好兑现’怎么办？如果大盘突然跳水，该股的回撤深度在哪里？）。会议目标不是达成一致，而是发现‘最坏的情况’。
股票基本信息：
- 股票代码：{stock_info.get('symbol', None)}
- 股票名称：{stock_info.get('name', None)}
- 当前价格：{stock_info.get('current_price', None)}
...
决策不仅仅是价位，更是‘场景触发’。必须给出明确的撤退信号：例如‘若开盘 30 分钟无量且跌破黄线，无论亏损多少必须清仓’。止损位必须参考质疑官提到的‘逻辑崩塌位’，而不仅仅是百分比。
股票信息：
- 股票代码：{stock_info.get('symbol', None)}
- 股票名称：{stock_info.get('name', None)}
- 当前价格：{stock_info.get('current_price', None)}
...
{{
    ...
    "position_size": "仓位建议",
    "risk_warning": "风险提示",
    "exit_trigger_condition": "描述什么样的盘口特征出现时必须立刻撤退（例如：放量跌破黄线）",
