...
def display_profit_growth():
    ...
    with col_monitor:
        st.write("")  # 占位
        if st.button("📊 策略监控", type="primary", width=True):
    ...
    **量化交易策略**：
    ...
    # 开始选股按钮
    if st.button("🚀 开始净利增长选股", type="primary", width=True):
    ...
    if 'profit_growth_stocks' in st.session_state and st.session_state.profit_growth_stocks is not None:
        ...
        # 发送钉钉通知
        st.markdown("---")
        if st.button("📲 发送钉钉通知", type="secondary", width=True):
...
def add_stock_to_monitor_button(stock_code: str, stock_name: str, price: float = None):
    ...
    if st.button(f"➕ 加入策略监控", key=button_key, width=True):
...
def display_monitoring_list(stocks: List[Dict]):
    ...
    for stock in stocks:
        with st.expander(f"📊 {stock['stock_code']} {stock['stock_name']}", expanded=False):
            ...
            with col3:
                if st.button("🗑️ 移除", key=f"remove_{stock['stock_code']}", width=True):
